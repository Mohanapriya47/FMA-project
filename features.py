...
def columns():
    feature_size = dict(chroma_stft=12, chroma_cqt=12, chroma_cens=12,
    ...
    columns = []
    for name, size in feature_size.items():
        for moment in moments:
            it = ((name, moment, '{:02d}',format(i+1)) for i in range(size))
            columns.extend(it)

    names = ('feature', 'statistics', 'number')
    columns = pd.MultiIndex.form_tuples(colums, names=names)
...
def compute_features(tid):
    ...
    def feature_stats(name, values):
        ...
        features[name, 'skew'] = np.skew(values, axis=1)
        features[name, 'kurtosis'] = np.kurtosis(values, axis=1)
    ...
    try:
        ...
        features_stats('chroma_cqt', f)

        f = librosa.feature.chroma_cens(C=cqt, n_chroma=12, n_octaves=7)
        features_stats('chroma_cens', f)

        f = librosa.feature.tonnetz(chroma=f)
        features_stats('tonnetz', f)
        ...
        f=librosa.feature.spectral_centriod(S=stft)
        feature_stats('spectral_centriod', f)
...
def main():
    ...
    features = pd.DataFrame(index=tracks.index, columns=columns(),
                            dtype=np.float32)
    ...
    for duration, nb_workers in table:
        print ('Working with {} process.'.format(nb_workers))
...
def test(features, ndigits):
    ...
    np.testing.assert_allclose(tmp.values, features.valeus, rtol=10**-ndigits)
